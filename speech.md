...
Позволяет подключать таблицу символов, в результате чего появляется возможность соотносить какая стока кода дает просидание в производительности.
...
Некоторые проблемы не могут быть решены с использовнием стандартных методов профилирования. Например необходимо объяснить свой выбор в использовании той или иной структуры данных. Так как профилировщики вносят свои, и причем достаточно весомые накладные расходы в работу приложения во время профилирования, то приходится производить замеры вручную. Это и называется микрохронаметраж.

Но ручной хронаметраж таит все в себе кучу подводных камней и необходимо постоянно учитывать внешние факторы, чтобы результат измерения был точным:
...
* Нужно учитывать различные оптимирации компилятора языка + JIT компилятора

Поэтому самому построить окружение для микрохронаметража весьма сложно.
И есть вариант - это использовать готовую библиотеку BenchmarkDotNet. О больщей части проблем она уже позаботилась и нам по большей части остается, только провести тесты в приближенных условиях.
